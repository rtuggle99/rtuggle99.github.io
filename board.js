...
const Board3D = (() => {
  ...
  function colorsForState(state) {
    ...
    // shading values, Light/Wood still read the same as before - the
    ...
    // The checkerboard's own light/dark squares - and their pips - always
    // shift with the Top shading value too, the same as the border,
    // blocks, and die: the whole board brightens or dims together as one
    // piece rather than the playing surface staying fixed while
    // everything raised above it shades.
    const topPct = shading.top;
    const tinted = hex => '#' + shadeHex(hex, -(100 - topPct)).toString(16).padStart(6, '0');
    const baseLight = custom ? c.light : themeVals.light;
    const baseDark = custom ? c.dark : themeVals.dark;
    const light = tinted(baseLight);
    const dark = tinted(baseDark);
    const pipLight = tinted(custom ? c.pipLight : themeVals.pipLight);
    const pipDark = tinted(custom ? c.pipDark : themeVals.pipDark);
    return {
      light, dark,
      pipLight, pipDark,
      goal: tinted(custom ? c.goal : themeVals.goal),
      goalBorder: tinted(custom ? c.goalBorder : '#003300'),
      goalPip: tinted(custom ? c.goalPip : (themeVals.goalPip || '#ffffff')),
  ...
  function makePipTexture(value, dotColor, bgColor) {
    ...
    // Exactly matches TikZ's \dicenum macro geometry (dot radius 1/6,
    // positions at +-0.5, both scaled by 0.4) converted into this
    // texture's fraction-of-the-0.6-unit-plane coordinate system -
    // dot size and spacing were previously chosen independently and
    // didn't share TikZ's proportions, so pips covered a visibly
    // different fraction of the cell between the two renderers.
    const r = size * 0.1111;
    const pts = {
      c: [0.5, 0.5], tl: [0.1667, 0.1667], tr: [0.8333, 0.1667],
      bl: [0.1667, 0.8333], br: [0.8333, 0.8333], ml: [0.1667, 0.5], mr: [0.8333, 0.5]
    };
    const layouts = {
      1: ['c'], 2: ['tl', 'br'], 3: ['bl', 'c', 'tr'],
      4: ['tl', 'tr', 'bl', 'br'], 5: ['tl', 'tr', 'c', 'bl', 'br'],
      6: ['tl', 'tr', 'ml', 'mr', 'bl', 'br']
    };
    // Rotated to match TikZ's own dicenum macro, which is drawn inside a
    // rotate=90 scope - without this, e.g. a 6 comes out as two columns
    // of three here but three columns of two in the exported TikZ.
    ctx.save();
  ...
  // Blocks need to be in the same render queue as pips (transparent) so
  // renderOrder can actually control which one draws on top - Three.js
  // splits opaque and transparent materials into separate passes and
  // always does opaque first regardless of renderOrder, so a plain
  // opaque block material would always end up "behind" a transparent
  // pip texture no matter what renderOrder said.
  const blockMat = hex => new THREE.MeshBasicMaterial({
  ...
  function blockFaceMats(shades) {
    return [
      blockMat(shades.right), blockMat(shades.left), blockMat(shades.top),
      blockMat(shades.left), blockMat(shades.back), blockMat(shades.front)
    ];
  }
  ...
  function rebuild(state) {
    ...
    state.blocked.forEach(key => {
      ...
      const m = new THREE.Mesh(geo, blockFaceMats(col.blockShades));
      ...
      // ---- shared face position/orientation definitions, used for both
      // the selected-face highlight ring and face-image overlays below
      const half = BLOCK_FOOTPRINT / 2;
      ...
      // ---- highlight ring on whichever face is selected for face-image
      // placement, so the "Highlight face" button actually shows
      // something - rounded, matching the die's own individual-face
      // highlight style (sharp edges are reserved for the group-wide
      // shading-direction highlight above).
      const selectedBlockFace = state.blockSelectedFace ? state.blockSelectedFace.get(key) : null;
      const selectedDef = selectedBlockFace && faceDefs[selectedBlockFace];
      if (selectedDef) {
        const ringSize = (selectedBlockFace === 'top' ? BLOCK_FOOTPRINT : Math.min(BLOCK_FOOTPRINT, slabH)) * 0.94;
        const ringGeo = new THREE.PlaneGeometry(ringSize, ringSize);
        const ringMat = new THREE.MeshBasicMaterial({
          map: getRoundedHighlightTexture(), transparent: true, depthTest: false, depthWrite: false
        });
        const ringMesh = new THREE.Mesh(ringGeo, ringMat);
        ringMesh.rotation.set(selectedDef.rotX, selectedDef.rotY, 0);
        ringMesh.position.set(...selectedDef.pos);
    ...
    {
      const z = PATH_Y;
      const topPct = (state.shading || { top: 100 }).top;
      ...
      // Double-sided so these are visible regardless of how the global
      // Z-flip on boardGroup affects winding order for a given shape -
      // flatSegment's own vertex order already accounts for it, but
      // that tuning doesn't automatically carry over to different
      // geometry (circles, the arrowhead), so this sidesteps the issue
      // directly rather than re-deriving the correct winding by hand.
      const pathMat = color => new THREE.MeshBasicMaterial({
        color,
        polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1
      });
      // Builds one continuous ribbon strip through a whole list of spine
      // points, sharing vertices at every joint - unlike chaining many
      // separate flatSegment rectangles, this can't show a gap or seam
      // where the direction changes slightly from one point to the next,
      // which is exactly what happens when approximating a curve with
      // many short straight pieces instead of one connected strip.
      function flatRibbon(points, halfWidth) {
        ...
        for (let i = 0; i < points.length; i++) {
          ...
          const nx = (-tz / tlen) * halfWidth, nz = (tx / tlen) * halfWidth;
          const p = points[i];
          lefts.push(new THREE.Vector3(p.x + nx, p.y, p.z + nz));
          rights.push(new THREE.Vector3(p.x - nx, p.y, p.z - nz));
        }
        const verts = [];
        for (let i = 0; i < points.length; i++) {
          verts.push(lefts[i].x, lefts[i].y, lefts[i].z);
          verts.push(rights[i].x, rights[i].y, rights[i].z);
        }
        const indices = [];
        for (let i = 0; i < points.length - 1; i++) {
          const l0 = i * 2, r0 = i * 2 + 1, l1 = i * 2 + 2, r1 = i * 2 + 3;
          indices.push(r0, l0, r1, l0, l1, r1);
        }
        const geo = new THREE.BufferGeometry();
      ...
      (state.paths || []).forEach(pathObj => {
        ...
        const style = { color: pathObj.color || '#8c2f27', thickness: pathObj.thickness != null ? pathObj.thickness : 2.2, endStyle: pathObj.endStyle || 'arrow' };
        const pathHex = shadeHex(style.color, -(100 - topPct));
        // Matches TikZ's actual physical line width: TikZ draws with
        // `line width=${thickness}pt`, and since 1 board cell equals 1
        // TikZ unit (1cm) with no extra scale factor, the same width in
        // board-cell units is thickness points converted to cm (1pt =
        // 0.03514cm), halved for the half-width used here.
        const halfWidth = style.thickness * 0.03514 / 2;
        const pts = seg.map(p => new THREE.Vector3(p.c + 0.5, z, p.r + 0.5));

        // True rounded corners, matching TikZ's rounded corners=4pt -
        // trims both segments approaching a bend and connects the trim
        // points with a smooth sampled arc using the original corner as
        // the Bezier control point, instead of approximating the corner
        // with a plain circle (which reads as a bulge, not a curve).
        // All the straight and curved pieces are collected into one
        // ordered spine and drawn as a single ribbon (see flatRibbon
        // above), so nothing has a seam at the joints.
        const roundR = halfWidth * 3;
        function arcPoints(before, corner, after, r) {
          const dirIn = before.clone().sub(corner).normalize();
          const dirOut = after.clone().sub(corner).normalize();
          const trimIn = corner.clone().addScaledVector(dirIn, r);
          const trimOut = corner.clone().addScaledVector(dirOut, r);
          const curve = new THREE.QuadraticBezierCurve3(trimIn, corner, trimOut);
          return curve.getPoints(8);
        }

        const isClosed = style.endStyle === 'closed' && pts.length >= 3;
        const cornerIdxStart = isClosed ? 0 : 1;
        const cornerIdxEnd = isClosed ? pts.length : pts.length - 1;
        const roundedCorner = new Map(); // corner index -> its arc sample points, in order
        for (let i = cornerIdxStart; i < cornerIdxEnd; i++) {
          const prevPt = pts[(i - 1 + pts.length) % pts.length];
          const nextPt = pts[(i + 1) % pts.length];
          const lenIn = pts[i].distanceTo(prevPt), lenOut = pts[i].distanceTo(nextPt);
          const r = Math.min(roundR, lenIn / 2, lenOut / 2);
          if (r < 1e-4) continue; // segments too short to round meaningfully - leave this corner sharp
          roundedCorner.set(i, arcPoints(prevPt, pts[i], nextPt, r));
        }
        // For an arrow ending, the ribbon should stop at the arrowhead's
        // own back edge, not the true tip - otherwise the full-width
        // ribbon extends into the arrowhead's pointed region and its
        // straight edges show past the arrowhead's outline instead of
        ...
        if (style.endStyle === 'arrow' && pts.length >= 2) {
          ...
          const notchLen = headLen * 0.55; // how far forward the back notch is pulled, in from the two back corners
          const backX = tip.x - ux * headLen, backZ = tip.z - uz * headLen;
          const notchX = tip.x - ux * notchLen, notchZ = tip.z - uz * notchLen;
          const nx = -uz, nz = ux;
          const leftX = backX + nx * headWidth, leftZ = backZ + nz * headWidth;
          const rightX = backX - nx * headWidth, rightZ = backZ - nz * headWidth;
          arrowGeom = { backPoint: new THREE.Vector3(notchX, tip.y, notchZ) };
          const geo = new THREE.BufferGeometry();
          geo.setAttribute('position', new THREE.BufferAttribute(new Float32Array([
            tip.x, tip.y, tip.z,      // 0: tip
            leftX, tip.y, leftZ,      // 1: left back corner
            notchX, tip.y, notchZ,    // 2: notch (pulled forward, concave back)
            rightX, tip.y, rightZ     // 3: right back corner
          ]), 3));
          geo.setIndex([0, 2, 1, 0, 3, 2]);
          arrowGeom.mesh = new THREE.Mesh(geo, pathMat(pathHex));
        }
        // Build the full ordered spine: original start point (or the
        // last corner's arc-end, for a closed loop), then each corner's
        // arc points (or the sharp vertex itself, if too short to
        // round) in turn, ending at the true final point (or the
        // arrowhead's back edge, if this path ends in an arrow).
        const spine = [];
        ...
        else if (arrowGeom) spine.push(arrowGeom.backPoint);
        else spine.push(pts[pts.length - 1]);
        const ribbonGeo = flatRibbon(spine, halfWidth);
        if (ribbonGeo) boardGroup.add(flatContentMesh(new THREE.Mesh(ribbonGeo, pathMat(pathHex))));
        // Round end caps, matching TikZ's line cap=round - the arrowhead
        // (if any) already covers the end visually, so only add a cap
        // there when there isn't one.
        if (style.endStyle !== 'closed') {
          const startCap = new THREE.Mesh(new THREE.CircleGeometry(halfWidth, 16), pathMat(pathHex));
          startCap.rotation.x = -Math.PI / 2;
          startCap.position.copy(pts[0]);
          boardGroup.add(flatContentMesh(startCap));
          if (style.endStyle !== 'arrow') {
            const endCap = new THREE.Mesh(new THREE.CircleGeometry(halfWidth, 16), pathMat(pathHex));
            endCap.rotation.x = -Math.PI / 2;
            endCap.position.copy(pts[pts.length - 1]);
            boardGroup.add(flatContentMesh(endCap));
          }
        }
        if (arrowGeom) boardGroup.add(flatContentMesh(arrowGeom.mesh));
      });
    }
    ...
    function computeStickerFitSize(s) {
      ...
      // mesh.rotation.z below is (90 - rot), a fixed +90 twist on top of
      // the user's rotation to lay the plane flat correctly - so the
      // world x/z axes actually end up swapped when that combined twist
      // is an odd multiple of 90, i.e. at rot 0/180, not rot 90/270.
      const swapped = rot === 0 || rot === 180;
      if (s.fit === 'contain') {
    ...
    // The crop-axis decision (crop the apparent/post-rotation X or Y)
    // is made first, then mapped onto the texture's natural U/V axis -
    // at a 90/270 rotation, apparent-X corresponds to natural-V (the
    // image's own height) and apparent-Y to natural-U (its width), not
    // the other way round, since the whole texture rotates with the mesh.
    function computeStickerUV(s) {
      const rot = s.rotation || 0;
      // Same baked +90 twist as computeStickerFitSize above - see the
      // comment there for why the swap happens at rot 0/180.
      const swapped = rot === 0 || rot === 180;
      const naturalW = s.naturalW || 1, naturalH = s.naturalH || 1;
      const apparentAspect = swapped ? naturalH / naturalW : naturalW / naturalH;
      const boxAspect = s.wCols / s.hRows;
      let repeatApparentX, repeatApparentY, offsetApparentX, offsetApparentY;
      if (apparentAspect > boxAspect) {
        // Apparently wider than the box - crop the apparent sides.
        const r = boxAspect / apparentAspect;
        repeatApparentX = r; repeatApparentY = 1;
        offsetApparentX = (1 - r) / 2; offsetApparentY = 0;
      } else {
        // Apparently taller than the box - crop the apparent top/bottom.
        const r = apparentAspect / boxAspect;
        repeatApparentX = 1; repeatApparentY = r;
        offsetApparentX = 0; offsetApparentY = (1 - r) / 2;
      }
      if (swapped) {
        return { repeatX: repeatApparentY, repeatY: repeatApparentX, offsetX: offsetApparentY, offsetY: offsetApparentX };
      }
      return { repeatX: repeatApparentX, repeatY: repeatApparentY, offsetX: offsetApparentX, offsetY: offsetApparentY };
    }
    (state.stickers || []).forEach(s => {
      ...
      if (cached) {
        ...
        // Multiplying by a grayscale material color tints the texture
        // toward black proportionally to the Top shading percentage,
        // without altering the image's own pixel content - matches the
        // same "everything on the board shades together" treatment
        // applied to the checkerboard, pips, and path above.
        const topPct = (state.shading || { top: 100 }).top;
        const tint = Math.max(0, Math.min(1, topPct / 100));
        const mat = new THREE.MeshBasicMaterial({
          map, color: new THREE.Color(tint, tint, tint),
          transparent: true, opacity: s.opacity != null ? s.opacity : 1,
          depthTest: false, depthWrite: false
        });
        const mesh = new THREE.Mesh(geo, mat);
        mesh.rotation.z = ((90 - (s.rotation || 0) + 360) % 360) * Math.PI / 180;
        mesh.scale.x = s.mirrored ? 1 : -1;
  ...
  // A transparent rounded amber ring, matching the die's own selected-face
  // highlight exactly, but standalone so it can be used as an overlay
  // plane anywhere an individual face is being highlighted - rounded
  // edges are reserved for this "one specific face" kind of highlight,
  // as opposed to wallHighlight's sharp-edged style used for the
  // group-wide shading-direction highlights.
  function makeRoundedHighlightTexture() {
    const size = 512, radius = 34, inset = 40;
    const c = document.createElement('canvas');
    c.width = c.height = size;
    const ctx = c.getContext('2d');
    ctx.lineWidth = 32;
  ...
  let roundedHighlightTextureCache = null;
  function getRoundedHighlightTexture() {
    if (!roundedHighlightTextureCache) roundedHighlightTextureCache = makeRoundedHighlightTexture();
    return roundedHighlightTextureCache;
  }
