...
const Board3D = (() => {
  ...
  function rebuild(state) {
    ...
    // ---- path(s) - flat ribbons embedded on the surface, not raised tubes ----
    {
      ...
      const doubleSidedPathMat = color => new THREE.MeshBasicMaterial({
      ...
      (state.paths || []).forEach(pathObj => {
        ...
        // round) in turn, ending at the true final point.
        const spine = [];
        ...
        if (ribbonGeo) boardGroup.add(flatContentMesh(new THREE.Mesh(ribbonGeo, doubleSidedPathMat(pathHex))));
        ...
        if (style.endStyle !== 'closed') {
          const startCap = new THREE.Mesh(new THREE.CircleGeometry(halfWidth, 16), doubleSidedPathMat(pathHex));
          startCap.rotation.x = -Math.PI / 2;
          startCap.position.copy(pts[0]);
          boardGroup.add(flatContentMesh(startCap));
          if (style.endStyle !== 'arrow') {
            const endCap = new THREE.Mesh(new THREE.CircleGeometry(halfWidth, 16), doubleSidedPathMat(pathHex));
        ...
        if (style.endStyle === 'arrow' && pts.length >= 2) {
          // A flat "Stealth"-style arrowhead at the final point, matching
          // the exported TikZ's Stealth tip shape exactly - not a plain
          // triangle, but one with a concave notch swept back from the
          // tip, which is what gives Stealth arrows their sleek look.
          ...
          geo.setIndex([0, 1, 2, 0, 2, 3]);
          boardGroup.add(flatContentMesh(new THREE.Mesh(geo, doubleSidedPathMat(pathHex))));
        }
      });
    }
    ...
    (state.stickers || []).forEach(s => {
      ...
      if (cached) {
        ...
        mesh.rotation.z = ((90 + (s.rotation || 0)) % 360) * Math.PI / 180;
