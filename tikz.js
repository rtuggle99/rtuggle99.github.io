  ...
  function generate(state, angles) {
    ...
    if (activePaths.length) {
      ...
      activePaths.forEach((pathObj, pIdx) => {
        ...
        const thickness = fmt(pathObj.thickness != null ? pathObj.thickness : 2.2);
        const colorMacro = `cPath${pIdx}Color`;
        pathText += `\\definecolor{${colorMacro}}{HTML}{${mixTowardBlack(pathObj.color || '#8c2f27', sh.top)}}\n`;
        if (pts.length >= 2) {
          if (pathObj.endStyle === 'arrow') {
            pathText += `\\draw[-{Stealth[length=${thickness}mm]}, line width=${thickness}pt, draw=${colorMacro}, line cap=round, line join=round, rounded corners=4pt]\n  ${pts.join(' -- ')};\n`;
          } else if (pathObj.endStyle === 'closed' && seg.length >= 3) {
            pathText += `\\draw[draw=${colorMacro}, opacity=1, line width=${thickness}pt, line cap=round, line join=round]\n  ${pts.join(' -- ')} -- cycle;\n`;
          } else {
            pathText += `\\draw[draw=${colorMacro}, opacity=1, line width=${thickness}pt, line cap=round, line join=round]\n  ${pts.join(' -- ')};\n`;
          }
        }
