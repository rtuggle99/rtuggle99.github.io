...
const TikzExport = (() => {
  ...
  function generate(state, angles) {
    ...
    if (state.stickers && state.stickers.some(s => s.fit === 'cover')) {
      // Used to measure each cover-fit image's real native size at
      // compile time (see the cover-fit sticker code below for why).
      out += '\\newlength{\\stickerImgW}\n\\newlength{\\stickerImgH}\n';
    }
    out += '\n\\begin{document}\n\n';
    ...
    if (state.stickers && state.stickers.length) {
      state.stickers.forEach(s => {
        ...
        let w, h, coverClipOpen = '', coverClipClose = '', measure = '';
        if (s.fit === 'contain') {
          ...
          // whichever axis overflows, with the crop-axis decision
          // re-mapped from apparent (post-rotation) to natural
          // (pre-rotation) space exactly the way board.js's
          // computeStickerUV does - at a 90/270 rotation, apparent-X
          // corresponds to the image's natural height and apparent-Y
          // to its natural width, not the other way round, since the
          // whole image rotates as one unit.
          ...
          // The crop amount has to be a FRACTION of the image's real
          // native size, not an absolute pixel count fed straight into
          // trim: graphicx's trim operates on the image's native
          // bounding box, which pdflatex derives from any DPI metadata
          // the file carries (a 300dpi photo's native box is far
          // smaller than its raw pixel count), so a pixel-valued trim
          // silently goes negative on any image that isn't ~72dpi -
          // which is most real photos. To get the fraction right
          // without knowing the DPI ahead of time, \settowidth /
          // \settoheight measure the image's actual native size as
          // pdflatex itself sees it, and the trim is computed from
          // that measurement via \dimexpr - confirmed correct against
          // a real 300dpi image via an actual pdflatex compile.
          const cropFrac = fmt((1 - r) / 2);
          ...
          measure = `\\settowidth{\\stickerImgW}{\\includegraphics{${s.filename}}}\n` +
            `\\settoheight{\\stickerImgH}{\\includegraphics{${s.filename}}}\n`;
          coverClipOpen = cropsNaturalWidth
            ? `trim={\\the\\dimexpr${cropFrac}\\stickerImgW\\relax} 0pt {\\the\\dimexpr${cropFrac}\\stickerImgW\\relax} 0pt, clip, `
            : `trim=0pt {\\the\\dimexpr${cropFrac}\\stickerImgH\\relax} 0pt {\\the\\dimexpr${cropFrac}\\stickerImgH\\relax}, clip, `;
        } else {
          w = userSwapped ? s.hRows : s.wCols;
          h = userSwapped ? s.wCols : s.hRows;
        }
        ...
        const darkenOverlay = darkAmount > 0.004
          ...
          : '';
        const text = measure +
          `\\begin{scope}[canvas is xy plane at z=${bh}, transform shape]\n` +
