  ...
  function generate(state, angles) {
    ...
    if (state.stickers && state.stickers.length) {
      state.stickers.forEach(s => {
        ...
        // fit='contain': compute the fitted size in world space, in terms
        // of the image's own natural aspect ratio versus the box, then
        // pre-swap for the node's local (pre-rotation) frame based on the
        // user's own rotation - the base +90 frame-alignment offset does
        // NOT itself cause a visual width/height swap here (confirmed via
        // a real compile at rotation=0), only the user's own 90/270 does.
