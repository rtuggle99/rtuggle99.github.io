...
(() => {
  const state = {
    ...
    paths: [{ points: [], color: '#8c2f27', thickness: 2.2, endStyle: 'arrow' }],
    activePathIndex: 0,
    pendingPathStyle: { color: '#8c2f27', thickness: 2.2, endStyle: 'arrow' },
  ...
  function countLayerableItems() {
    const hasPath = state.paths.some(seg => seg.points.length > 0);
  ...
  function currentPathObj() {
    if (state.activePathIndex == null || state.activePathIndex >= state.paths.length) {
      state.activePathIndex = state.paths.length - 1;
    }
    return state.paths[state.activePathIndex];
  }
  function currentPath() { return currentPathObj().points; }

  let nextStickerId = 1;
  function placeSticker(clickC, clickR) {
    if (!state.pendingImage) return;
    const { wCols, hRows } = state.stickerSize;
    // The clicked cell is the sticker's min col (left edge) and its
    // max row (bottom edge, in c/r index terms) - so the anchor's c
    // matches the click directly, while r sits back from the click by
    // the sticker's own height.
    const c = clickC;
    const r = clickR - hRows + 1;
    if (r < 0 || c + wCols > state.cols) return;
    state.stickers.push({
      id: nextStickerId++,
  ...
  function clampToGrid() {
    ...
    state.paths = state.paths
      .map(p => ({ ...p, points: p.points.filter(pt => inRange(pt.c, pt.r)) }))
      .filter(p => p.points.length > 0);
    if (state.paths.length === 0) state.paths = [{ points: [], color: state.pendingPathStyle.color, thickness: state.pendingPathStyle.thickness, endStyle: state.pendingPathStyle.endStyle }];
  ...
  function setCols(value) {
    ...
    if (!Number.isFinite(n) || n < 1 || n > 100) return; // ignore invalid/mid-edit input rather than rebuilding on it
    state.cols = n;
    $('#cols').value = n;
    $('#cols-number').value = n;
    clampToGrid();
  ...
  $('#cols-number').addEventListener('input', e => setCols(e.target.value));
  $('#rows').addEventListener('input', e => setRows(e.target.value));
  $('#rows-number').addEventListener('input', e => setRows(e.target.value));
  ...
  $('#btn-clear-paths').addEventListener('click', () => {
    state.paths = [{ points: [], color: state.pendingPathStyle.color, thickness: state.pendingPathStyle.thickness, endStyle: state.pendingPathStyle.endStyle }];
  ...
  $('#btn-new-path').addEventListener('click', () => {
    ...
    state.paths.push({ points: [], color: state.pendingPathStyle.color, thickness: state.pendingPathStyle.thickness, endStyle: state.pendingPathStyle.endStyle });
    state.activePathIndex = state.paths.length - 1;
    rebuild();
    refreshPathColorUI();
  });

  // The color/thickness/end-style controls edit whichever path is
  // currently being drawn (the last one) - each path keeps its own
  // style, so different traced routes can have different colors. The
  // controls also update pendingPathStyle, used as the starting style
  // for the next new path via "Start new segment".
  function refreshPathColorUI() {
    const cur = currentPathObj();
    $('#c-pathcolor').value = cur.color;
    $('#path-thickness').value = Math.round(cur.thickness * 10);
    $('#path-thickness-val').textContent = cur.thickness.toFixed(1);
    $('#path-endstyle').value = cur.endStyle;
    renderPathList();
  }
  function renderPathList() {
    ...
    if (!list) return;
    list.innerHTML = '';
    const withPoints = state.paths.filter(p => p.points.length > 0);
    if (withPoints.length === 0) {
      list.innerHTML = '<p class="help">No traced routes yet.</p>';
      return;
    }
    state.paths.forEach((p, idx) => {
      ...
      const isActive = p === currentPathObj();
      const label = document.createElement('span');
      label.textContent = `Route ${idx + 1} (${p.points.length} squares)${isActive ? ' (active)' : ''}`;
      if (!isActive) {
        label.style.cursor = 'pointer';
        label.title = 'Click to make this the active route';
      ...
      colorInput.addEventListener('input', e => {
        ...
        if (isActive) state.pendingPathStyle.color = e.target.value;
        Board3D.rebuild(state);
        if (isActive) $('#c-pathcolor').value = e.target.value;
      });
  ...
  $('#c-pathcolor').addEventListener('input', e => {
    currentPathObj().color = e.target.value;
    state.pendingPathStyle.color = e.target.value;
    rebuild();
  });
  $('#path-thickness').addEventListener('input', e => {
    const t = Number(e.target.value) / 10;
    currentPathObj().thickness = t;
    state.pendingPathStyle.thickness = t;
    $('#path-thickness-val').textContent = t.toFixed(1);
    rebuild();
  });
  ...
  $('#path-endstyle').addEventListener('change', e => {
    currentPathObj().endStyle = e.target.value;
    state.pendingPathStyle.endStyle = e.target.value;
    rebuild();
  });
  ...
  function renderStickerList() {
    ...
    state.stickers.forEach(s => {
      ...
      const label = document.createElement('span');
      label.textContent = `${s.filename} (${s.hRows}\u00d7${s.wCols} at row ${s.c}, column ${s.r})`;
  ...
  function renderDiceList() {
    ...
    state.dice.forEach(dieObj => {
      ...
      row.className = 'sticker-row';
      const label = document.createElement('span');
      label.textContent = `Die at row ${dieObj.position.c}, column ${dieObj.position.r}`;
      ...
      nextFaceBtn.className = 'btn btn-small face-highlight-btn';
  ...
  function renderBlockList() {
    ...
    [...state.blocked].sort().forEach(k => {
      ...
      row.className = 'sticker-row';
      const label = document.createElement('span');
      label.textContent = `Block at row ${c}, column ${r}`;
      row.append(label);
    ...
    switch (state.tool) {
      ...
      case 'goal':
        if (state.goal && state.goal.c === c && state.goal.r === r) state.goal = null;
          ...
          // Default to a pip of 6 on a freshly-placed goal, matching the
          // old built-in behavior - but this is just an ordinary pip
          // stamp like any other, so the Pip tool can freely change or
          // clear it afterward, and it's skipped if this square already
          // has a pip value from before.
          if (!state.labels.get(k)) state.labels.set(k, 6);
        }
        renderBlockList();
        break;
      case 'label':
  ...
  // A persistent corner banner for the known SwiftLaTeX server outage,
  // separate from the regular transient status banner above. Since the
  // outage doesn't resolve itself moment to moment, repeatedly flashing
  // a one-off error every time the idle auto-render loop retries (every
  // few hundred ms) is misleading and noisy - this instead shows once,
  // stays put, and the auto-render loop stops retrying entirely while
  // it's up (see the setInterval loop below). A manual "Download PDF"
  // click can still clear it if the server has actually come back.
  const swiftlatexDownEl = $('#swiftlatex-down-banner');
  const swiftlatexDownTextEl = $('#swiftlatex-down-text');
  let swiftlatexDown = false;
  // Once the user dismisses the banner, it stays dismissed for the rest
  // of this page load even if a later attempt fails again - the point
  // is to stop nagging, not to reappear the next time something retries.
  // swiftlatexDown itself still tracks the real state underneath (so the
  // auto-render loop keeps not wasting effort), this only controls
  // whether the banner is shown.
  let swiftlatexBannerDismissed = false;
  function setSwiftlatexDown(isDown) {
    swiftlatexDown = isDown;
    if (!isDown) swiftlatexBannerDismissed = false; // server's back - next outage should show fresh
    if (isDown && !swiftlatexBannerDismissed) {
      swiftlatexDownTextEl.textContent = "SwiftLaTeX's server is currently down, so \"Render TikZ when idle\" and \"Download PDF\" are currently non-functional. \"Download TikZ\" still works, and that file can be compiled elsewhere (e.g. Overleaf) in the meantime.";
      swiftlatexDownEl.classList.remove('hidden');
    } else {
      swiftlatexDownEl.classList.add('hidden');
    }
  }
  $('#swiftlatex-down-close').addEventListener('click', () => {
    swiftlatexBannerDismissed = true;
    swiftlatexDownEl.classList.add('hidden');
  });
  function looksLikeSwiftlatexDown(err) {
    const msg = (err && err.message) || '';
    return /format file|Download Failed|can't find the format/i.test(msg);
  }
  ...
  $('#btn-pdf').addEventListener('click', async () => {
    ...
    try {
      ...
      if (swiftlatexDown) setSwiftlatexDown(false); // server's back
    } catch (err) {
      console.error(err);
      if (looksLikeSwiftlatexDown(err)) {
        setSwiftlatexDown(true);
      } else {
        const msg = (err && err.message) || '';
        setStatus('Could not compile a PDF. You can still use "Download TikZ" and compile it yourself (e.g. on Overleaf). ' + (msg ? msg.split('\n')[0] : ''), true);
      }
    } finally {
      btn.disabled = false;
    }
  ...
  function stateSignature(angles) {
    return JSON.stringify({
      ...
      labels: [...state.labels.entries()].sort(), paths: state.paths,
  ...
  async function showExact() {
    ...
    try {
      ...
      if (looksLikeSwiftlatexDown(err)) {
        ...
        const msg = (err && err.message) || '';
        setStatus('Could not compile the exact render. ' + (msg ? msg.split('\n')[0] : ''), true);
      }
    } finally {
      compiling = false;
    }
  ...
  setInterval(() => {
    if (!autoEnabled || showingExact || compiling || swiftlatexDown) return;
